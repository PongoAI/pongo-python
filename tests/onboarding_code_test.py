...
def pongo_demo():
    sub_org_id = pongo_client.create_sub_org(
        sub_org_name="Tutorial Organization"
    ).json()["sub_org_id"]
    ...
    while True:
        job_status = pongo_client.get_job(job_id=job_id, sub_org_id=sub_org_id).json()[
            "job"
        ]["job_status"]
    ...
    search_response = pongo_client.search(
        query="What is User1's favorite color?", sub_org_id=sub_org_id
    )
